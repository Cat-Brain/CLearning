...
void IncreaseVertexListList(VertexListList* l, uint count)
{
	...
	AddToUIntList(&l->offsets, FindTotalOfVertexListList(*l) + count);

	l->l = (Vertex*)realloc(l->l, FindTotalOfVertexListList(*l) * sizeof(Vertex));
}
...
void RemoveFromVertexListList(VertexListList* l, uint index)
{
	l->offsets.count--;

	for (uint i = index; i < l->offsets.count; i++)
	{
